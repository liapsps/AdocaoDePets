...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    // Função para exibir os pets no grid
    function displayPets(pets) {
        ...
        pets.forEach((pet) => {
            ...
            card.innerHTML = `
                ...
                <p class="pet-categoria">Categoria: ${categoria && categoria.nome ? categoria.nome : "N/A"}</p>
                <p class="pet-descricao">${descricao || ""}</p>
                <a class="info-button" href="../pages/detalhes_pet.html?id=${documentId}">Mais informações</a>
            `;
            petGrid.appendChild(card);
        });
    }

    // Função para popular os filtros com os dados dos pets
    ...
    // Função para aplicar os filtros aos pets
    function applyFilters() {
        ...
        const filteredPets = petsData.filter((pet) => {
            const catName = pet.categoria && pet.categoria.nome ? pet.categoria.nome.toLowerCase() : "";
            const spName = pet.raca ? pet.raca.toLowerCase() : "";
            const sizeMatch = !sizeFilter || pet.tamanho === sizeFilter;
            const categoryMatch = !categoryFilter || catName.includes(categoryFilter);
    ...
    // Adiciona evento de clique ao botão de limpar filtros
    if (filterClearBtn) {
