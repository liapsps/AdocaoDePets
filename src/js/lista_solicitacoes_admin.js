	...
	async function fetchSolicitacoes() {
		try {
			...
			if (!solicitacoes || solicitacoes.length === 0) {
				solicitacoesContainer.innerHTML = `<p>Nenhuma solicitação de adoção encontrada.</p>`;
				return;
			}
			...
			solicitacoes.forEach(solic => {
				...
				const { id, justificativa, situacao, pet, adotante } = solic;
				...
				// Caso o objeto adotante esteja populado, podemos exibir o nome dele (opcional)
				...
				card.innerHTML = `
					...
					<div class="card-buttons">
						<button class="aceitar-button" ${situacao !== 'Pendente' ? 'disabled' : ''}>Aceitar</button>
						<button class="recusar-button" ${situacao !== 'Pendente' ? 'disabled' : ''}>Recusar</button>
					</div>
				`;
				// Botão "Aceitar"
				...
				aceitarButton.addEventListener('click', async () => {
					if (confirm("Tem certeza que deseja aceitar esta solicitação?")) {
						try {
							...
							// Atualiza o pet para estabelecer o relacionamento com o adotante.
							// Supondo que o objeto "adotante" esteja presente na solicitação
							if (pet && adotante) {
							...
							card.querySelector('.situacao-text').textContent = "Confirmada";
